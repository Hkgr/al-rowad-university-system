...
2. Run `00_preflight_manual.sql`. Continue only when `can_apply=1`; the required operational identities are exactly `registrar` and `exam.board`, and each must already reference an employee.
3. Run `01_apply_manual.sql`. It idempotently seeds `PRES` when absent, upserts all 58 units in the official chart, moves the two employees to units `732` and `735`, and grants both users an active university scope rooted at `PRES`.
4. Run `02_verify_manual.sql`. Do not deploy unless `official_chart_exact_58`, both staff checks, and `OVERALL` all return `PASS`, and the mismatch detail query returns no rows.
