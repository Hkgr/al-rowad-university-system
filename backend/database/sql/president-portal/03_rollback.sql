-- Removes only tagged portal read permissions/mappings/module. Never removes the existing role, accounts, scopes or history.
-- Disable portal traffic first; inspect foreign mappings. No business data rollback.
...
START TRANSACTION;
DELETE rp FROM alrowad_uni_rust.role_permissions rp JOIN alrowad_uni_rust.permissions p ON p.permission_id=rp.permission_id JOIN alrowad_uni_rust.roles r ON r.role_id=rp.role_id
WHERE @ready AND r.role_code='university_president' AND p.permission_code IN ('president_portal.access','president_portal.dashboard.view','president_portal.colleges.view','president_portal.students.view','president_portal.exams.view','president_portal.staff.view','president_portal.leadership.view','president_portal.followup.view','president_portal.reports.view') AND p.description LIKE '%[president-portal]%';
DELETE FROM alrowad_uni_rust.permissions WHERE @ready AND permission_code IN ('president_portal.access','president_portal.dashboard.view','president_portal.colleges.view','president_portal.students.view','president_portal.exams.view','president_portal.staff.view','president_portal.leadership.view','president_portal.followup.view','president_portal.reports.view') AND description LIKE '%[president-portal]%' AND NOT EXISTS (SELECT 1 FROM alrowad_uni_rust.role_permissions rp WHERE rp.permission_id=alrowad_uni_rust.permissions.permission_id);
DELETE FROM alrowad_uni_rust.system_modules WHERE @ready AND module_code='president_portal' AND description LIKE '%[president-portal]%' AND NOT EXISTS (SELECT 1 FROM alrowad_uni_rust.permissions p WHERE p.module_id=alrowad_uni_rust.system_modules.module_id);
COMMIT;
SELECT 'OVERALL' AS section, IF(@ready,'ROLLED_BACK_OR_ABSENT','BLOCKED') AS result;
