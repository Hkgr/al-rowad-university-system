-- Read-only verification. Existing president role and all pre-existing permissions remain unchanged.
...
SET @mapped := (SELECT COUNT(DISTINCT p.permission_code) FROM alrowad_uni_rust.permissions p JOIN alrowad_uni_rust.role_permissions rp ON rp.permission_id=p.permission_id JOIN alrowad_uni_rust.roles r ON r.role_id=rp.role_id WHERE r.role_code='university_president' AND p.permission_code IN ('president_portal.access','president_portal.dashboard.view','president_portal.colleges.view','president_portal.students.view','president_portal.exams.view','president_portal.staff.view','president_portal.leadership.view','president_portal.followup.view','president_portal.reports.view'));
SELECT 'OPERATOR_READINESS' AS section, COUNT(DISTINCT u.user_id) AS active_scoped_president_accounts
FROM alrowad_uni_rust.users u JOIN alrowad_uni_rust.account_statuses ast ON ast.account_status_id=u.account_status_id
JOIN alrowad_uni_rust.user_roles ur ON ur.user_id=u.user_id JOIN alrowad_uni_rust.roles r ON r.role_id=ur.role_id
JOIN alrowad_uni_rust.user_access_scopes s ON s.user_id=u.user_id JOIN alrowad_uni_rust.organizational_units ou ON ou.organizational_unit_id=s.scope_id
WHERE ast.status_code='active' AND ur.is_active=1 AND r.is_active=1 AND r.role_code='university_president' AND s.is_active=1 AND s.scope_type='university' AND ou.unit_code='PRES';
SELECT 'OVERALL' AS section, IF(@ready AND @mapped=9,'PASS','FAIL') AS result, @mapped AS mapped_read_permissions;
