...
SET @uq_identity := IF(@identity_unique_exists >= 1, 1, 0);
SET @idx_status := IF(
    @db_ready = 1,
...
SET @events_idx_lookup_ok := @idx_event_type;
...
SELECT 'unique_identity' AS check_name, IF(@uq_identity = 1, 'PASS', 'FAIL') AS result, @identity_unique_exists AS identity_unique_exists;
SELECT 'event_table' AS check_name, IF(@events_contract_ok = 1, 'PASS', 'FAIL') AS result;
