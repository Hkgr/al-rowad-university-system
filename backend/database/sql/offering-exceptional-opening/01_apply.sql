...
SET @requests_expected_cols := IF(
    ...
    (
        SELECT COUNT(*)
        FROM (
            SELECT 'course_offering_exception_request_id' AS column_name
            ...
            UNION ALL SELECT 'snapshot_course_id'
            UNION ALL SELECT 'snapshot_academic_program_id'
            UNION ALL SELECT 'snapshot_academic_year_id'
            UNION ALL SELECT 'snapshot_semester_id'
            UNION ALL SELECT 'snapshot_department_id'
            ...
            UNION ALL SELECT 'superseded_reason'
...
SET @reviews_expected_cols := IF(
    ...
    (
        SELECT COUNT(*)
        FROM (
            SELECT 'course_offering_exception_review_id' AS column_name
            UNION ALL SELECT 'course_offering_exception_request_id'
            UNION ALL SELECT 'submission_version'
...
SET @events_expected_cols := IF(
    ...
    (
        SELECT COUNT(*)
        FROM (
            SELECT 'course_offering_exception_event_id' AS column_name
            UNION ALL SELECT 'course_offering_exception_request_id'
...
SET @sql := IF(
    ...
    'CREATE TABLE `alrowad_uni_rust`.`course_offering_exception_requests` (
        `course_offering_exception_request_id` INT NOT NULL AUTO_INCREMENT,
        ...
        `snapshot_course_id` INT NOT NULL,
        `snapshot_academic_program_id` INT NOT NULL,
        `snapshot_academic_year_id` INT NOT NULL,
        `snapshot_semester_id` INT NOT NULL,
        `snapshot_department_id` INT NULL,
        `submitted_at` TIMESTAMP NULL,
        `approved_at` TIMESTAMP NULL,
        `materialized_at` TIMESTAMP NULL,
        `superseded_at` TIMESTAMP NULL,
        `superseded_by_request_id` INT NULL,
        `superseded_reason` VARCHAR(64) NULL,
        `created_at` TIMESTAMP NULL,
        `updated_at` TIMESTAMP NULL,
        PRIMARY KEY (`course_offering_exception_request_id`),
        UNIQUE KEY `uq_coer_current_slot` (`course_offering_id`, `current_slot`),
        KEY `idx_coer_status` (`status`),
        KEY `idx_coer_requested_by` (`requested_by_user_id`),
        KEY `idx_coer_submitted_at` (`submitted_at`),
        KEY `idx_coer_offering_status` (`course_offering_id`, `status`),
        CONSTRAINT `fk_coer_course_offering` FOREIGN KEY (`course_offering_id`) REFERENCES `alrowad_uni_rust`.`course_offerings` (`course_offering_id`),
        CONSTRAINT `fk_coer_requested_by` FOREIGN KEY (`requested_by_user_id`) REFERENCES `alrowad_uni_rust`.`users` (`user_id`),
        CONSTRAINT `fk_coer_superseded_by` FOREIGN KEY (`superseded_by_request_id`) REFERENCES `alrowad_uni_rust`.`course_offering_exception_requests` (`course_offering_exception_request_id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=''[phase6-offering-exceptional-opening]''',
    'SELECT ''SKIPPED_REQUESTS'' AS apply_result'
);
PREPARE phase6_coer_stmt FROM @sql;
EXECUTE phase6_coer_stmt;
DEALLOCATE PREPARE phase6_coer_stmt;

SET @sql := IF(
    @apply_ready = 1 AND @reviews_state = 'ABSENT',
    'CREATE TABLE `alrowad_uni_rust`.`course_offering_exception_reviews` (
        `course_offering_exception_review_id` INT NOT NULL AUTO_INCREMENT,
        `course_offering_exception_request_id` INT NOT NULL,
        ...
        `created_at` TIMESTAMP NULL,
        `updated_at` TIMESTAMP NULL,
        PRIMARY KEY (`course_offering_exception_review_id`),
        UNIQUE KEY `uq_coerv_request_authority_version` (`course_offering_exception_request_id`, `review_authority`, `submission_version`),
        KEY `idx_coerv_authority_status` (`review_authority`, `status`),
        CONSTRAINT `fk_coerv_request` FOREIGN KEY (`course_offering_exception_request_id`) REFERENCES `alrowad_uni_rust`.`course_offering_exception_requests` (`course_offering_exception_request_id`),
        CONSTRAINT `fk_coerv_reviewer` FOREIGN KEY (`reviewed_by_user_id`) REFERENCES `alrowad_uni_rust`.`users` (`user_id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=''[phase6-offering-exceptional-opening]''',
    'SELECT ''SKIPPED_REVIEWS'' AS apply_result'
);
PREPARE phase6_coerv_stmt FROM @sql;
EXECUTE phase6_coerv_stmt;
DEALLOCATE PREPARE phase6_coerv_stmt;

SET @sql := IF(
    @apply_ready = 1 AND @events_state = 'ABSENT',
    'CREATE TABLE `alrowad_uni_rust`.`course_offering_exception_events` (
        `course_offering_exception_event_id` INT NOT NULL AUTO_INCREMENT,
        `course_offering_exception_request_id` INT NOT NULL,
        ...
        PRIMARY KEY (`course_offering_exception_event_id`),
        KEY `idx_coee_request_created` (`course_offering_exception_request_id`, `created_at`),
        CONSTRAINT `fk_coee_request` FOREIGN KEY (`course_offering_exception_request_id`) REFERENCES `alrowad_uni_rust`.`course_offering_exception_requests` (`course_offering_exception_request_id`),
        CONSTRAINT `fk_coee_actor` FOREIGN KEY (`actor_user_id`) REFERENCES `alrowad_uni_rust`.`users` (`user_id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=''[phase6-offering-exceptional-opening]''',
...
SELECT sm.module_id, 'course_offerings.exceptional_open.view', 'View exceptional course offering opening',
       'Read exceptional-opening workflow queues and details [phase6-offering-exceptional-opening]',
...
WHERE @apply_ready = 1
  ...
  AND NOT EXISTS (
      ...
      WHERE existing.permission_code = 'course_offerings.exceptional_open.view'
  );

INSERT INTO `alrowad_uni_rust`.`permissions` (
    module_id, permission_code, permission_name, description, is_active, created_at, updated_at
)
SELECT sm.module_id, 'course_offerings.exceptional_open.request', 'Request exceptional course offering opening',
       'Dean-side create and resubmit of exceptional-opening requests [phase6-offering-exceptional-opening]',
       1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM `alrowad_uni_rust`.`system_modules` sm
WHERE @apply_ready = 1
  ...
  AND NOT EXISTS (
      SELECT 1 FROM `alrowad_uni_rust`.`permissions` existing
      WHERE existing.permission_code = 'course_offerings.exceptional_open.request'
  );

INSERT INTO `alrowad_uni_rust`.`permissions` (
    module_id, permission_code, permission_name, description, is_active, created_at, updated_at
)
SELECT sm.module_id, 'course_offerings.exceptional_open.review_scientific', 'Scientific exceptional opening review',
       'Scientific VP approve or return exceptional-opening requests [phase6-offering-exceptional-opening]',
       1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM `alrowad_uni_rust`.`system_modules` sm
WHERE @apply_ready = 1
  ...
  AND NOT EXISTS (
      SELECT 1 FROM `alrowad_uni_rust`.`permissions` existing
      WHERE existing.permission_code = 'course_offerings.exceptional_open.review_scientific'
  );

INSERT INTO `alrowad_uni_rust`.`permissions` (
    module_id, permission_code, permission_name, description, is_active, created_at, updated_at
)
SELECT sm.module_id, 'course_offerings.exceptional_open.review_administrative', 'Administrative exceptional opening review',
       'Administrative VP approve or return exceptional-opening requests [phase6-offering-exceptional-opening]',
       1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM `alrowad_uni_rust`.`system_modules` sm
WHERE @apply_ready = 1
  ...
  AND NOT EXISTS (
      SELECT 1 FROM `alrowad_uni_rust`.`permissions` existing
      WHERE existing.permission_code = 'course_offerings.exceptional_open.review_administrative'
  );
...
JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_code = 'course_offerings.exceptional_open.view'
WHERE @apply_ready = 1
...
SELECT r.role_id, p.permission_id, CURRENT_TIMESTAMP
FROM `alrowad_uni_rust`.`roles` r
JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_code = 'course_offerings.exceptional_open.request'
WHERE @apply_ready = 1
...
SELECT r.role_id, p.permission_id, CURRENT_TIMESTAMP
FROM `alrowad_uni_rust`.`roles` r
JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_code = 'course_offerings.exceptional_open.review_scientific'
WHERE @apply_ready = 1
...
SELECT r.role_id, p.permission_id, CURRENT_TIMESTAMP
FROM `alrowad_uni_rust`.`roles` r
JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_code = 'course_offerings.exceptional_open.review_administrative'
WHERE @apply_ready = 1
...
SET @phase6_complete := IF(
    ...
    AND (SELECT COUNT(*) FROM `alrowad_uni_rust`.`permissions` WHERE permission_code = 'course_offerings.exceptional_open.view' AND is_active = 1) = 1
    AND (SELECT COUNT(*) FROM `alrowad_uni_rust`.`permissions` WHERE permission_code = 'course_offerings.exceptional_open.request') = 1
    AND (SELECT COUNT(*) FROM `alrowad_uni_rust`.`permissions` WHERE permission_code = 'course_offerings.exceptional_open.request' AND is_active = 1) = 1
    AND (SELECT COUNT(*) FROM `alrowad_uni_rust`.`permissions` WHERE permission_code = 'course_offerings.exceptional_open.review_scientific') = 1
    AND (SELECT COUNT(*) FROM `alrowad_uni_rust`.`permissions` WHERE permission_code = 'course_offerings.exceptional_open.review_scientific' AND is_active = 1) = 1
    AND (SELECT COUNT(*) FROM `alrowad_uni_rust`.`permissions` WHERE permission_code = 'course_offerings.exceptional_open.review_administrative') = 1
    AND (SELECT COUNT(*) FROM `alrowad_uni_rust`.`permissions` WHERE permission_code = 'course_offerings.exceptional_open.review_administrative' AND is_active = 1) = 1
    AND EXISTS (
        SELECT 1 FROM `alrowad_uni_rust`.`roles` r
        JOIN `alrowad_uni_rust`.`role_permissions` rp ON rp.role_id = r.role_id
        JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_id = rp.permission_id
        WHERE r.role_code = 'dean' AND p.permission_code = 'course_offerings.exceptional_open.view'
    )
    AND EXISTS (
        SELECT 1 FROM `alrowad_uni_rust`.`roles` r
        JOIN `alrowad_uni_rust`.`role_permissions` rp ON rp.role_id = r.role_id
        JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_id = rp.permission_id
        WHERE r.role_code = 'dean' AND p.permission_code = 'course_offerings.exceptional_open.request'
    )
    AND EXISTS (
        SELECT 1 FROM `alrowad_uni_rust`.`roles` r
        JOIN `alrowad_uni_rust`.`role_permissions` rp ON rp.role_id = r.role_id
        JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_id = rp.permission_id
        WHERE r.role_code = 'vice_president_scientific' AND p.permission_code = 'course_offerings.exceptional_open.view'
    )
    AND EXISTS (
        SELECT 1 FROM `alrowad_uni_rust`.`roles` r
        JOIN `alrowad_uni_rust`.`role_permissions` rp ON rp.role_id = r.role_id
        JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_id = rp.permission_id
        WHERE r.role_code = 'vice_president_scientific' AND p.permission_code = 'course_offerings.exceptional_open.review_scientific'
    )
    AND EXISTS (
        SELECT 1 FROM `alrowad_uni_rust`.`roles` r
        JOIN `alrowad_uni_rust`.`role_permissions` rp ON rp.role_id = r.role_id
        JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_id = rp.permission_id
        WHERE r.role_code = 'vice_president_administrative' AND p.permission_code = 'course_offerings.exceptional_open.view'
    )
    AND EXISTS (
        SELECT 1 FROM `alrowad_uni_rust`.`roles` r
        JOIN `alrowad_uni_rust`.`role_permissions` rp ON rp.role_id = r.role_id
        JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_id = rp.permission_id
        WHERE r.role_code = 'vice_president_administrative' AND p.permission_code = 'course_offerings.exceptional_open.review_administrative'
    )
    AND NOT EXISTS (
        SELECT 1 FROM `alrowad_uni_rust`.`roles` r
        JOIN `alrowad_uni_rust`.`role_permissions` rp ON rp.role_id = r.role_id
        ...
        WHERE r.role_code = 'dean'
          AND p.permission_code IN ('course_offerings.exceptional_open.review_scientific', 'course_offerings.exceptional_open.review_administrative')
    )
    AND NOT EXISTS (
        SELECT 1 FROM `alrowad_uni_rust`.`roles` r
        JOIN `alrowad_uni_rust`.`role_permissions` rp ON rp.role_id = r.role_id
        JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_id = rp.permission_id
        WHERE r.role_code = 'vice_president_scientific' AND p.permission_code = 'course_offerings.exceptional_open.review_administrative'
    )
    AND NOT EXISTS (
        SELECT 1 FROM `alrowad_uni_rust`.`roles` r
        JOIN `alrowad_uni_rust`.`role_permissions` rp ON rp.role_id = r.role_id
        JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_id = rp.permission_id
        WHERE r.role_code = 'vice_president_administrative' AND p.permission_code = 'course_offerings.exceptional_open.review_scientific'
    )
    AND NOT EXISTS (
        SELECT 1 FROM `alrowad_uni_rust`.`roles` r
        JOIN `alrowad_uni_rust`.`role_permissions` rp ON rp.role_id = r.role_id
    ...
    AND NOT EXISTS (
        ...
        JOIN `alrowad_uni_rust`.`role_permissions` rp ON rp.role_id = r.role_id
        JOIN `alrowad_uni_rust`.`permissions` p ON p.permission_id = rp.permission_id
        WHERE p.permission_code = 'course_offerings.exceptional_open.review_scientific'
          AND r.role_code <> 'vice_president_scientific'
    )
    AND NOT EXISTS (
        SELECT 1 FROM `alrowad_uni_rust`.`roles` r
