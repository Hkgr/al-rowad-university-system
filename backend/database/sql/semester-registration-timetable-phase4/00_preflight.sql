...
SET @srt4_target_state := IF(@srt4_target_tables=0,'ABSENT',IF(@srt4_target_total_columns=10 AND @srt4_target_columns=10 AND @srt4_target_shape=10 AND @srt4_target_pk=1 AND @srt4_target_indexes=2 AND @srt4_target_fks=2 AND @srt4_target_checks=3 AND @srt4_target_comment=1,'COMPATIBLE','CONFLICTING'));
SET @srt4_ready := (@srt4_database=1 AND @srt4_tables=18 AND @srt4_columns=69 AND @srt4_signed_parents=4 AND @srt4_phase1_permission=1 AND @srt4_dean_mapping>=1 AND @srt4_registration_status=1 AND @srt4_target_state<>'CONFLICTING');
...
SELECT 'SIGNED_KEYS_AND_RBAC' report_section,IF(@srt4_signed_parents=4 AND @srt4_phase1_permission=1 AND @srt4_dean_mapping>=1 AND @srt4_registration_status=1,'PASS','FAIL') result,CONCAT('signed_keys=',@srt4_signed_parents,'/4; manage_permission=',@srt4_phase1_permission,'; dean_mapping=',@srt4_dean_mapping) detail;
