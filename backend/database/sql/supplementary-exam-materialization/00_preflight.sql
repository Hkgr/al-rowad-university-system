-- READ ONLY. Run as one phpMyAdmin SQL-tab execution and inspect the final row.
...
SET @rbac_tables_ready := (
    ...
    WHERE table_schema = 'alrowad_uni_rust'
      AND table_type = 'BASE TABLE'
      AND table_name IN ('roles', 'permissions', 'role_permissions', 'user_roles', 'system_modules')
);
...
PREPARE phase6_preflight_rbac FROM @sql;
EXECUTE phase6_preflight_rbac;
DEALLOCATE PREPARE phase6_preflight_rbac;
...
SET @mat_compatible := @mat_exists AND @mat_table_ready AND @mat_columns_ready
    AND @mat_primary_ready AND @mat_json_ready AND @mat_indexes_ready AND @mat_fks_ready;
       ...
       AND SUM(
           CASE
               WHEN column_name = 'supplementary_exam_materialization_event_id'
                   THEN data_type = 'int' AND column_type NOT LIKE '%unsigned%' AND is_nullable = 'NO' AND column_default IS NULL AND LOWER(extra) = 'auto_increment'
               WHEN column_name IN (
                   'supplementary_exam_materialization_id', 'supplementary_exam_offering_id',
                   'supplementary_exam_registration_id', 'source_submission_version', 'actor_user_id'
               ) THEN data_type = 'int' AND column_type NOT LIKE '%unsigned%' AND is_nullable = 'NO' AND column_default IS NULL AND extra = ''
...
SET @event_indexes_ready := (
    ...
    WHERE (non_unique = 0 AND index_columns IN (
        ...
        'supplementary_exam_offering_id', 'supplementary_exam_registration_id',
...
SET @event_compatible := @event_exists AND @event_table_ready AND @event_columns_ready
    AND @event_primary_ready AND @event_indexes_ready AND @event_fks_ready;
...
FROM (
    SELECT 10 AS sort_order, 'DATABASE' AS report_section, IF(@database_ready, 'PASS', 'FAIL') AS result
    UNION ALL SELECT 20, 'DEPENDENCY_TABLES', IF(@dependency_tables_ready, 'PASS', 'FAIL')
    UNION ALL SELECT 30, 'DEPENDENCY_COLUMNS', IF(@dependency_columns_ready AND @canonical_result_ready, 'PASS', 'FAIL')
    UNION ALL SELECT 35, 'OPTIONAL_RESULT_ANNOUNCED_AT', @result_announced_at_classification
    UNION ALL SELECT 40, 'DRIFT_TIMESTAMPS', IF(@drift_timestamps_ready, 'PASS', 'FAIL')
    UNION ALL SELECT 50, 'SIGNED_PARENT_IDS', IF(@signed_parent_ids_ready, 'PASS', 'FAIL')
    UNION ALL SELECT 55, 'PARENT_PRIMARY_KEYS', IF(@parent_primary_keys_ready, 'PASS', 'FAIL')
    UNION ALL SELECT 60, 'RBAC_DEPENDENCIES', IF(@rbac_dependencies_ready, 'PASS', 'FAIL')
    UNION ALL SELECT 70, 'supplementary_exam_materializations', @mat_classification
    UNION ALL SELECT 80, 'supplementary_exam_materialization_events', @event_classification
    UNION ALL SELECT 90, @phase6_permission, @permission_classification
    UNION ALL SELECT 99, 'OVERALL', IF(@preflight_ready, 'READY', 'BLOCKED')
) report
ORDER BY sort_order;
