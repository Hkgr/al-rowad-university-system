...
SELECT 'active_fmf_college_10' check_name, IF(COUNT(*)=1,'PASS','FAIL') result, COUNT(*) actual
FROM colleges WHERE college_code='FMF' AND college_id=10 AND college_name='كلية العلوم الإدارية والمالية' AND is_active=1;
...
SELECT 'dean_is_only_assigned_role' check_name,
 IF(COUNT(*)=1 AND SUM(r.role_code='dean')=1,'PASS','FAIL') result, COUNT(*) actual
FROM user_roles ur JOIN roles r ON r.role_id=ur.role_id WHERE ur.user_id=@test_user_id;
...
SELECT 'no_other_active_scope' check_name, IF(COUNT(*)=0,'PASS','FAIL') result, COUNT(*) actual
FROM user_access_scopes WHERE user_id=@test_user_id AND is_active=1
 AND NOT (scope_type='college' AND scope_id=10);
...
SELECT 'OVERALL' check_name,
 ...
 AND (SELECT COUNT(*) FROM employees WHERE employee_number='TEMP-DEAN-FMF-2026')=1
 ...
 AND (SELECT COUNT(*) FROM user_roles ur JOIN roles r ON r.role_id=ur.role_id
      WHERE ur.user_id=@test_user_id)=1
