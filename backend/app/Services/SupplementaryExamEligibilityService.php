...
class SupplementaryExamEligibilityService
{
    ...
    public function activeValidDeferral(StudentCourseRegistration $registration): ?SupplementaryExamTheoreticalDeferral
    {
        ...
        if(!$row)return null; $registration->loadMissing(['registrationStatus','resultStatus','studentCourseResult.resultStatus']); $parts=$this->parts($registration);
        $mark=$this->partMark($registration,$parts,'practical'); $practical=$parts->contains('component_type','practical');
        return $row->offering?->isOpen() && $row->offering?->period?->status==='announced'
            && in_array($registration->registrationStatus?->status_code,StudentCourseRegistration::HISTORICAL_ATTEMPT_STATUSES,true)
            && $this->sourceAllowed($row->offering,$registration) && !$this->deprived($registration)
            && (!$practical||($mark!==null&&$mark>=(float)$this->grades->defaultGradingPolicy()->minimum_practical_mark)) ? $row : null;
    }

    public function declare(User $user,int $offeringId,int $registrationId): SupplementaryExamTheoreticalDeferral
    {
        $this->assertStudent($user);
        return DB::transaction(function()use($user,$offeringId,$registrationId){
            ...
            if((int)$r->student_id!==(int)$user->student_id)$this->fail('The registration is not owned by this student.','deferral_not_owned',403);
            CourseOfferingLock::lock((int)$r->course_offering_id);
            ...
            $offering=SupplementaryExamOffering::query()->with('period')->lockForUpdate()->findOrFail($offeringId);
            if(SupplementaryExamTheoreticalDeferral::query()->where('student_course_registration_id',$registrationId)->where('current_slot',1)->lockForUpdate()->exists())$this->fail('A current deferral already exists.','deferral_already_current');
            $blockers=$this->declarationBlockers($offering,$r,$parts,$theoryApproval); if($blockers!==[])$this->fail('Theoretical deferral is not available.',$blockers[0]);
            $row=SupplementaryExamTheoreticalDeferral::query()->where('supplementary_exam_offering_id',$offeringId)->where('student_course_registration_id',$registrationId)->lockForUpdate()->first();
            $type=$row?'redeclaration':'declared';$from=$row?->status;$values=['status'=>'declared','current_slot'=>1,'declared_by_user_id'=>$user->user_id,'declared_at'=>now(),'cancelled_by_user_id'=>null,'cancelled_at'=>null,'cancellation_reason'=>null,'superseded_at'=>null,'supersede_reason'=>null];
            $row?$row->update($values):$row=SupplementaryExamTheoreticalDeferral::query()->create($values+['supplementary_exam_offering_id'=>$offeringId,'student_course_registration_id'=>$registrationId]);
            $this->event($row,$type,$from,'declared',(int)$user->user_id,null);return $row->fresh('offering.period');
        },3);
    }
    ...
    {
        $this->assertStudent($user);
        return DB::transaction(function()use($user,$deferral,$reason){$row=SupplementaryExamTheoreticalDeferral::query()->with('offering.period')->lockForUpdate()->findOrFail($deferral->getKey());
            $r=StudentCourseRegistration::query()->lockForUpdate()->findOrFail($row->student_course_registration_id);if((int)$r->student_id!==(int)$user->student_id)$this->fail('Not owned.','deferral_not_owned',403);
            CourseOfferingLock::lock((int)$r->course_offering_id);$approval=GradePartApproval::query()->where('course_offering_id',$r->course_offering_id)->where('component_type','theoretical')->lockForUpdate()->first();
            $hasMark=StudentGradeComponent::query()->where('student_course_registration_id',$r->getKey())->whereHas('gradeComponent',fn($q)=>$q->where('component_type','theoretical'))->whereNotNull('mark')->lockForUpdate()->exists();
            if($row->status!=='declared'||$row->current_slot!=1||!$row->offering?->isOpen()||$row->offering?->period?->status!=='announced'||$hasMark||($approval&&!in_array($approval->status,['draft','returned'],true)))$this->fail('Cannot cancel.','deferral_cannot_cancel');
            $row->update(['status'=>'cancelled','current_slot'=>null,'cancelled_by_user_id'=>$user->user_id,'cancelled_at'=>now(),'cancellation_reason'=>$reason]);$this->event($row,'cancelled','declared','cancelled',(int)$user->user_id,$reason);return $row->fresh();},3);
    }
