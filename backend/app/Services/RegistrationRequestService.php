...
class RegistrationRequestService
{
    ...
    public function studentWorkspace(Student $student, ?int $semesterId = null): array
    {
        ...
        $semester = $semesterId !== null
            ? $openSemesters->firstWhere('semester_id', $semesterId)
            : null;
    ...
    public function approve(User $user, StudentRegistrationRequest $request): array
    {
        ...
        try {
            return DB::transaction(function () use ($user, $request): array {
                ...
                $locked->loadMissing(['items.courseOffering.course', 'student']);
            ...
            throw new RegistrationRequestException(
                ...
                409,
                'registration_request_approval_failed',
                [['course_offering_id' => 0, 'reason' => 'already_registered']]
            );
        }
    }
    ...
    private function hoursFor(
        ...
        $registeredIds = $this->registration->currentOfferingIds($student);
        $requestHours = $this->requestHours($request, $registeredIds);
        $projected = (int) $base['registered_hours'] + $requestHours;
        $max = (int) $base['max_allowed_hours'];

        return [
            'registered_hours' => (int) $base['registered_hours'],
            'request_hours' => $requestHours,
            'projected_hours' => $projected,
            'max_allowed_hours' => $max,
            'remaining_before_request' => max($max - (int) $base['registered_hours'], 0),
            'remaining_after_approval' => max($max - $projected, 0),
        ];
    }
