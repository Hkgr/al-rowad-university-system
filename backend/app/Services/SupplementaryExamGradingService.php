...
class SupplementaryExamGradingService
{
    ...
    private function rosterPayload(SupplementaryExamOffering $offering): array
    {
        ...
        return ['offering'=>$offering,'workflow_status'=>$submission?->status ?? 'waiting','submission'=>$submission,'roster'=>$registrations->map(function($r){$practical=$r->originalRegistration?->studentCourseResult?->practical_total;$theory=$r->gradeResult?->theoretical_mark;$preview=$theory===null?null:$this->grades->buildCalculation((float)$theory,$practical===null?null:(float)$practical);return ['supplementary_exam_registration_id'=>$r->getKey(),'student'=>$r->student,'preserved_practical_mark'=>$practical,'supplementary_theoretical_mark'=>$theory,'result_status'=>$r->gradeResult?->status,'submission_version'=>$r->gradeResult?->submission_version,'preview'=>$preview,'official_record_materialized'=>false];})->all()];
    }
