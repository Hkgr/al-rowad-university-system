...
class ExecutiveReportQueryRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        ...
        $validator->after(function (Validator $v): void {
            ...
            // Collection callbacks receive value AND key; internal is_array accepts only one argument.
            $total=collect([(array)$this->input('filters',[]),(array)$this->input('period',[]),(array)$this->input('comparison.baseline.filters',[]),(array)$this->input('comparison.baseline.period',[])])->sum(fn($scope)=>collect($scope)->filter(fn($value)=>is_array($value))->sum(fn($values)=>count($values)));
