...
class SupplementaryExamRegistrationOfficeController extends Controller
{
 ...
 public function index(Request $r,int|string $period){$this->service->ready();$u=$r->user();abort_unless($u->effectivePermissions()->contains(SupplementaryExamRegistrationGovernance::VIEW)||$u->hasRoleCode('super_admin'),403);$q=SupplementaryExamRegistration::query()->with(['student','offering.period','offering.course','offering.academicProgram','originalRegistration.courseOffering.semester'])->where('status','registered')->where('current_slot',1)->whereHas('offering',fn($x)=>$x->where('supplementary_exam_period_id',(int)$period));$rows=$q->get()->filter(fn($x)=>$this->scope->canAccessStudent($u,$x->student)&&$this->scope->canAccessProgram($u,(int)$x->offering->academic_program_id))->values();$status=$rows->first()?->offering?->period?->status;return response()->json(['period_status'=>$status,'list_status'=>$status==='registration_closed'?'fixed':'draft','data'=>$rows]);}
}
