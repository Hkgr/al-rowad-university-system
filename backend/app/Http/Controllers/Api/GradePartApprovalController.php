...
class GradePartApprovalController extends Controller
{
    ...
    public function show(int $approval, Request $request, AcademicAuthorizationService $auth, GradePartWorkflowService $service): JsonResponse { $auth->assertExaminationCommittee($request->user()); $record = $service->find($request->user(), $approval); return $this->success(['approval' => (new GradePartApprovalResource($record))->resolve($request), 'workflow' => $service->workflow($record->course_offering_id)]); }
