...
class VicePresidencyCourseOfferingExceptionController extends Controller
{
        ...
        if ($queue === 'pending') {
            $query->whereHas('reviews', function ($reviews) use ($authority): void {
                $reviews
                    ...
                    ->where('status', ExceptionalOpeningWorkflow::REVIEW_PENDING)
                    ->whereColumn(
                        'course_offering_exception_reviews.submission_version',
                        'course_offering_exception_requests.submission_version'
                    );
            });
        } elseif ($queue === 'returned') {
            $query->where('status', ExceptionalOpeningWorkflow::STATUS_RETURNED);
        } elseif ($queue === 'approved') {
            $query->where('status', ExceptionalOpeningWorkflow::STATUS_APPROVED);
        }
    ...
    public function approveScientific(Request $request, CourseOfferingExceptionRequest $courseOfferingExceptionRequest): JsonResponse
    {
        $updated = $this->workflow->approveScientific($request->user(), $courseOfferingExceptionRequest);

        return $this->ok(
            (new CourseOfferingExceptionRequestResource($updated))->resolve($request),
            'تمت الموافقة العلمية على الفتح الاستثنائي.'
        );
    }

    public function returnScientific(Request $request, CourseOfferingExceptionRequest $courseOfferingExceptionRequest): JsonResponse
    {
        $validated = $request->validate([
        ...
        return $this->ok(
            (new CourseOfferingExceptionRequestResource($updated))->resolve($request),
            'أُعيد طلب الفتح الاستثنائي إلى العميد.'
        );
    }

    public function approveAdministrative(Request $request, CourseOfferingExceptionRequest $courseOfferingExceptionRequest): JsonResponse
    {
        $updated = $this->workflow->approveAdministrative($request->user(), $courseOfferingExceptionRequest);

        return $this->ok(
            (new CourseOfferingExceptionRequestResource($updated))->resolve($request),
            'تمت الموافقة الإدارية على الفتح الاستثنائي.'
        );
    }

    public function returnAdministrative(Request $request, CourseOfferingExceptionRequest $courseOfferingExceptionRequest): JsonResponse
    {
        $validated = $request->validate([
            'reason' => ['required', 'string', 'min:1', 'max:1000'],
        ]);
