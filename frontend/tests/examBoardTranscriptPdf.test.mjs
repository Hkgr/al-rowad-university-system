...
const academicRecordPresentation = readFileSync(new URL('../src/features/academic-record/lib/academicRecordPresentation.js', import.meta.url), 'utf8')
const exportAction = readFileSync(new URL('../src/features/academic-record/components/TranscriptPdfExportAction.jsx', import.meta.url), 'utf8')
...
test('grade sheet is search-only and the independent record page refreshes before PDF generation', () => {
  ...
  assert.match(academicRecord, /TranscriptPdfExportAction endpoint=\{endpoint\}/)
  assert.match(exportAction, /const response = await apiRequest\(endpoint\)/)
  assert.match(exportAction, /exportTranscriptPdf\(\{ academicRecord: response\.data \}\)/)
  ...
  assert.match(exportAction, /const \[error, setError\]/)
  assert.match(exportAction, /if \(exporting\.current\) return/)
  assert.match(exportAction, /جاري إنشاء الملف\.\.\./)
  assert.match(exportAction, /استخراج كشف العلامات الإلكتروني/)
  assert.equal(academicRecord.includes('/cgpa'), false)
  assert.match(exportAction, /TRANSCRIPT_EXPORT_ERROR/)
})
