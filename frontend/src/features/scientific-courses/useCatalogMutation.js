...
export default function useCatalogMutation({ currentPath, onSaved, onBusy, onBlocked, onUnauthorized }) {
  ...
  async function write(path, method, payload) {
    ...
    try { const result = await catalogWrite(path, method, payload); if (alive.current) onSaved(result) }
    catch (e) {
  ...
  async function inspect() {
    ...
    try { const current = await catalogRead(currentPath); if (alive.current) setState(s => ({ ...s, current, checking: false })) }
