import { alignComparison, detailColumns, formatMetric, reportRowKey } from '../presentation'
...
function periodText(period) {
  ...
  if (period.type === 'academic') return `السنة ${period.academic_year_ids?.join('، ') || '؟'}${period.semester_ids?.length ? ` — الفصل ${period.semester_ids.join('، ')}` : ''}`
  return 'الوضع الحالي'
}
...
export default function ReportResults({ report, applied, labels, stale, loading, onRefresh, onPage, onSort }) {
  ...
  return <div className="space-y-5">
    ...
    {report.comparison?.available && <div className="grid gap-2 rounded-2xl border border-primary/15 bg-white p-4 text-sm sm:grid-cols-2"><p><strong>التقرير الأساسي:</strong> {scopeText(report.scope, labels)} — {periodText(report.period)}</p><p><strong>{report.comparison.type === 'selected_scopes' ? 'النطاقات المقارنة' : 'خط الأساس'}:</strong> {report.comparison.type === 'selected_scopes' ? scopeText(report.comparison.scope, labels) : `${scopeText(report.comparison.scope, labels)} — ${periodText(report.comparison.period)}`}</p></div>}
