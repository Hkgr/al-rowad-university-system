...
function ReviewCard({ title, review }) {
  return (
    <div className="bg-primary/[0.03] border border-primary/10 rounded-[14px] px-4 py-3.5">
      ...
      {review?.reason ? (
        <p className="text-[12.5px] text-amber-800 mt-2 whitespace-pre-wrap">{review.reason}</p>
      ) : null}
...
export default function TeachingAssignmentDetail({ office }) {
  ...
  const canDecide = authority === 'administrative'
    ? hasPermission(PERMISSIONS.teachingAssignmentsReviewAdministrative)
    : hasPermission(PERMISSIONS.teachingAssignmentsReviewScientific)
  ...
  async function approve() {
    ...
    const path = authority === 'administrative'
      ? `/v1/vice-presidency/teaching-assignments/${id}/administrative/approve`
      : `/v1/vice-presidency/teaching-assignments/${id}/scientific/approve`
    try {
  ...
  async function returnToDean() {
    ...
    setNotice('')
    const path = authority === 'administrative'
      ? `/v1/vice-presidency/teaching-assignments/${id}/administrative/return`
      : `/v1/vice-presidency/teaching-assignments/${id}/scientific/return`
    try {
  ...
  return (
    <div className="space-y-5 py-6 px-2" dir="rtl">
      <Link to={`${basePath}/teaching-assignments`} className="inline-flex items-center gap-2 text-[13px] font-semibold text-text-gray">
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="bg-white border border-primary/12 rounded-[14px] px-4 py-3.5">
              <p className="text-[11.5px] text-text-light font-semibold mb-1">المدرس المقترح</p>
              <p className="text-[15px] font-extrabold text-text-dark">{facultyName(row.proposed_faculty_member)}</p>
              <p className="text-[12px] text-text-gray">{row.proposed_faculty_member?.academic_rank || '—'}</p>
              <p className="text-[12px] text-text-light">{row.proposed_faculty_member?.home_unit?.unit_name || ''}</p>
            </div>
            <div className="bg-white border border-primary/12 rounded-[14px] px-4 py-3.5">
              <p className="text-[11.5px] text-text-light font-semibold mb-1">المدرس المعتمد</p>
              <p className="text-[15px] font-extrabold text-text-dark">{facultyName(row.effective_faculty_member)}</p>
