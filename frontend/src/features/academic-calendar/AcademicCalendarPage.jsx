...
import { eventColor, eventsForDay, eventVersion, fromUniversityInput, isRegistrationDeadlineType, monthBounds, monthCells, statusBadgeKind, toUniversityInput, UNIVERSITY_TIME_ZONE, universityDateKey, withOptionalChangeReason } from './calendarUtils'
...
function EventForm({ mode, event, catalog, defaultYearId, busy, onClose, onSubmit }) {
  ...
  const usesRegistrationDeadlines = isRegistrationDeadlineType(selectedEventType)
  const isReplacementRegistration = selectedEventType?.event_type_code === 'course_registration_replacement'
  const registrationStartLabel = isReplacementRegistration ? 'بداية نافذة استبدال المقررات' : 'بداية تسجيل الطلاب'
  const studentDeadlineLabel = isReplacementRegistration ? 'نهاية اختيار الطالب للبديل' : 'نهاية تسجيل الطلاب'
  ...
  const submit = () => {
    const payload = withOptionalChangeReason({
      ...
      ends_at: fromUniversityInput(usesRegistrationDeadlines ? form.advisor_approval_ends_at : form.ends_at),
      is_enforcement: usesRegistrationDeadlines ? true : form.is_enforcement,
    }, reasonRequired)
    if (usesRegistrationDeadlines) {
  ...
  return <Modal title={mode === 'create' ? 'إنشاء مسودة حدث' : replacement ? 'مسودة بديلة للحدث المنشور' : 'تعديل المسودة'} onClose={onClose} wide>
    ...
    <form onSubmit={e => { e.preventDefault(); submit() }} className="grid gap-4 md:grid-cols-2">
      ...
      <label className="text-[13px] font-bold text-text-dark">{usesRegistrationDeadlines ? registrationStartLabel : 'البداية'} <span className="font-normal text-text-light">(بتوقيت الجامعة)</span><input required type="datetime-local" value={form.starts_at} onChange={e => set('starts_at', e.target.value)} className={fieldClass} /></label>
      {usesRegistrationDeadlines ? <>
        <label className="text-[13px] font-bold text-text-dark">{studentDeadlineLabel} <span className="font-normal text-text-light">(بتوقيت الجامعة)</span><input required type="datetime-local" min={form.starts_at} value={form.student_registration_ends_at} onChange={e => set('student_registration_ends_at', e.target.value)} className={fieldClass} /></label>
      ...
      <label className="flex items-center gap-2 text-[13px] font-bold text-text-dark md:col-span-2"><input type="checkbox" disabled={usesRegistrationDeadlines} checked={usesRegistrationDeadlines || form.is_enforcement} onChange={e => set('is_enforcement', e.target.checked)} /> نافذة تنفيذية</label>
...
export default function AcademicCalendarPage() {
  ...
  return <section dir="rtl" className="pb-10">
    ...
    {selected && !formMode && <Modal title={eventVersion(selected).title} onClose={() => setSelected(null)} wide><div className="space-y-4 text-[13px] text-text-dark">
      ...
      {isRegistrationDeadlineType(selected.event_type) && <div className="grid gap-2 rounded-[10px] border border-emerald-200 bg-emerald-50 p-4 text-emerald-950 sm:grid-cols-2"><p><strong>{selected.event_type.event_type_code === 'course_registration_replacement' ? 'نهاية اختيار الطالب للبديل:' : 'نهاية تسجيل الطلاب:'}</strong> {dateLabel.format(new Date(eventVersion(selected).student_registration_ends_at || eventVersion(selected).ends_at))} {timeLabel.format(new Date(eventVersion(selected).student_registration_ends_at || eventVersion(selected).ends_at))}</p><p><strong>نهاية اعتماد المرشد:</strong> {dateLabel.format(new Date(eventVersion(selected).advisor_approval_ends_at || eventVersion(selected).ends_at))} {timeLabel.format(new Date(eventVersion(selected).advisor_approval_ends_at || eventVersion(selected).ends_at))}</p></div>}
