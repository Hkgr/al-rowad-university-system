...
export default function DeanConfirmDialog({
  title,
  warning,
  confirmLabel,
  cancelLabel = 'إلغاء',
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="dean-confirm-title"
      onClick={event => {
    ...
    >
      <div className="w-full sm:max-w-[520px] max-h-[96vh] overflow-y-auto bg-white rounded-t-[18px] sm:rounded-[18px] shadow-2xl">
        <div className="flex items-center justify-between border-b border-primary/10 px-5 py-4 sticky top-0 bg-white z-10">
          <h3 id="dean-confirm-title" className="text-[16px] font-black text-text-dark">{title}</h3>
          <button
            type="button"
        ...
        <div className="px-5 py-4 space-y-4">
          {children}
          {warning && (
            <p className={`text-[13px] font-semibold rounded-[12px] px-3.5 py-3 ${
              confirmTone === 'danger'
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-primary/10">
          ...
          >
            ...
            <span>{busy ? 'جاري الحفظ' : confirmLabel}</span>
          </button>
        </div>
      </div>
    </div>
