...
function buildEdits(data) {
  ...
  for (const row of data?.students ?? []) for (const part of Object.keys(PARTS)) {
    for (const component of row.marks?.[part] ?? []) result[editKey(row.registration_id, component.grade_component_id)] = { value: component.mark ?? '', dirty: false }
  }
  return result
}
...
export default function ProfessorGradesPage() {
  ...
  const componentDefinitions = useMemo(() => {
    const found = new Map()
    for (const row of rows) for (const component of row.marks?.[selectedPart] ?? []) found.set(String(component.grade_component_id), component)
    return [...found.values()]
  }, [rows, selectedPart])
  const dirtyKeys = useMemo(() => Object.keys(edits).filter(key => edits[key]?.dirty), [edits])
  const selectedDirtyKeys = useMemo(() => {
    if (!selectedPart) return []
    const ids = new Set(rows.flatMap(row => (row.marks?.[selectedPart] ?? []).map(component => editKey(row.registration_id, component.grade_component_id))))
    return dirtyKeys.filter(key => ids.has(key))
  }, [dirtyKeys, rows, selectedPart])
  ...
  function rowValidation(row) {
    return (row.marks?.[selectedPart] ?? []).map(component => {
  ...
  async function handleSave() {
    ...
    const dirtyRows = rows.filter(row => !row.is_deprived && (row.marks?.[part] ?? []).some(component => edits[editKey(row.registration_id, component.grade_component_id)]?.dirty))
    ...
    const results = await Promise.all(validRows.map(async row => {
      const components = (row.marks?.[part] ?? []).map(component => ({
    ...
    setEdits(current => {
      const next = { ...current }
      for (const { row } of successful) for (const component of row.marks?.[part] ?? []) next[editKey(row.registration_id, component.grade_component_id)] = { ...next[editKey(row.registration_id, component.grade_component_id)], dirty: false }
      return next
    })
  ...
  const canSave = partState?.can_edit === true && selectedDirtyKeys.length > 0 && !saving && !submitting
  const canSubmit = partState?.required === true && partState.can_submit === true && !['submitted', 'approved'].includes(partState.status) && selectedDirtyKeys.length === 0 && !saving && !submitting
  ...
  return <>
    ...
    {selectedId && loadError && <Alert text={loadError} />}
    {workflow && <>
      ...
      {rows.length === 0 ? <Empty text="لا يوجد طلاب مسجلون في هذه المادة" /> : selectedPart && <div className="bg-white border border-primary/12 rounded-[16px] overflow-hidden shadow-[0_2px_10px_rgba(26,46,16,0.05)] mb-4"><div className="overflow-x-auto"><table className="w-full min-w-[720px] border-collapse text-[13px]" dir="rtl"><thead><tr className="bg-[#fafaf8]"><th className="px-3 py-3 text-center text-[11px] text-text-light">الرقم الجامعي</th><th className="px-3 py-3 text-right text-[11px] text-text-light">اسم الطالب</th>{componentDefinitions.map((component, index) => <th key={component.grade_component_id} className="px-3 py-3 text-center text-[11px] text-text-light">{PARTS[selectedPart].short}{componentDefinitions.length > 1 ? ` ${index + 1}` : ''} / {component.max_mark}</th>)}{selectedPart === 'theoretical' && <th className="px-3 py-3 text-center text-[11px] text-text-light">العملي (للقراءة فقط)</th>}<th className="px-3 py-3 text-center text-[11px] text-text-light">حالة الحفظ</th></tr></thead><tbody>
        {rows.map(row => { const components = row.marks?.[selectedPart] ?? []; const errors = rowValidation(row); const dirty = components.some(component => edits[editKey(row.registration_id, component.grade_component_id)]?.dirty); const editable = partState?.can_edit === true && !row.is_deprived; return <tr key={row.registration_id} className="border-t border-primary/6"><td className="px-3 py-3 text-center font-mono text-[11px]">{row.student?.student_number || '—'}</td><td className="px-3 py-3 text-right"><div className="font-semibold text-text-dark">{studentName(row.student)}</div>{row.is_deprived && <Badge text="محروم" />}</td>{componentDefinitions.map(definition => { const component = components.find(item => Number(item.grade_component_id) === Number(definition.grade_component_id)); if (!component) return <td key={definition.grade_component_id} className="px-3 py-3 text-center">—</td>; const index = components.indexOf(component); return <MarkInput key={definition.grade_component_id} value={edits[editKey(row.registration_id, component.grade_component_id)]?.value ?? ''} max={component.max_mark} error={errors[index]} dirty={edits[editKey(row.registration_id, component.grade_component_id)]?.dirty} disabled={!editable || saving || submitting} onChange={value => updateMark(row.registration_id, component.grade_component_id, value)} /> })}{selectedPart === 'theoretical' && <PracticalReference row={row} status={workflow.parts?.practical?.status} />}<td className="px-3 py-3 text-center">{dirty ? <span className="text-amber-700 font-bold text-[11px]">غير محفوظ</span> : <span className="text-green-700 font-bold text-[11px]"><FaCheck className="inline ml-1" />محفوظ</span>}</td></tr> })}
      </tbody></table></div></div>}
