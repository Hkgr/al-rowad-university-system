...
export default function ProfessorGradesPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setNotice(refreshed
        ? { type: 'success', text: `تم إرسال علامات الجزء ${PARTS[part].short} إلى هيئة الامتحانات.` }
  ...
  const canSave = !refreshRequired && !loadError && !loading && partState?.can_edit === true && selectedDirtyKeys.length > 0 && !saving && !submitting
  const canSubmit = !refreshRequired && !loadError && !loading && partState?.required === true && partState.can_submit === true && !['submitted', 'approved'].includes(partState.status) && selectedDirtyKeys.length === 0 && !saving && !submitting
  ...
  return <>
    ...
    {workflow && <>
      <OfferingContext offering={selectedOffering} workflow={workflow} />
      <div className="grid grid-cols-2 max-[600px]:grid-cols-1 gap-3 mb-4" dir="rtl">
        {Object.keys(PARTS).map(part => <PartStatus key={part} part={part} state={workflow.parts?.[part]} compact />)}
      </div>
      ...
      {partState?.review_notes && <ReviewNotes part={selectedPart} notes={partState.review_notes} />}
      {notice && <Notice notice={notice} />}
      {refreshRequired && <RetryAlert text="الحقول مقفلة احترازيًا حتى تأكيد الحالة الجديدة من الخادم." disabled={loading || saving || submitting} onRetry={() => loadWorkflow(selectedId)} />}
      {rows.length === 0 ? <Empty text="لا يوجد طلاب مسجلون في هذه المادة" /> : selectedPart && <div className="bg-white border border-primary/12 rounded-[16px] overflow-hidden shadow-[0_2px_10px_rgba(26,46,16,0.05)] mb-4"><div className="overflow-x-auto"><table className="w-full min-w-[720px] border-collapse text-[13px]" dir="rtl"><thead><tr className="bg-[#fafaf8]"><th className="px-3 py-3 text-center text-[11px] text-text-light">الرقم الجامعي</th><th className="px-3 py-3 text-right text-[11px] text-text-light">اسم الطالب</th>{componentDefinitions.map((component, index) => <th key={component.grade_component_id} className="px-3 py-3 text-center text-[11px] text-text-light">{PARTS[selectedPart].short}{componentDefinitions.length > 1 ? ` ${index + 1}` : ''} / {component.max_mark}</th>)}{selectedPart === 'theoretical' && <th className="px-3 py-3 text-center text-[11px] text-text-light">العملي (للقراءة فقط)</th>}<th className="px-3 py-3 text-center text-[11px] text-text-light">حالة الحفظ</th></tr></thead><tbody>
        {rows.map(row => { const errors = rowValidation(row); const dirty = componentDefinitions.some(component => edits[editKey(row.registration_id, component.grade_component_id)]?.dirty); const editable = !refreshRequired && !loadError && !loading && partState?.can_edit === true && !row.is_deprived; return <tr key={`${workflow.course_offering_id}:${row.registration_id}`} className="border-t border-primary/6"><td className="px-3 py-3 text-center font-mono text-[11px]">{row.student?.student_number || '—'}</td><td className="px-3 py-3 text-right"><div className="font-semibold text-text-dark">{studentName(row.student)}</div>{row.is_deprived && <Badge text="محروم" />}</td>{componentDefinitions.map((component, index) => <MarkInput key={component.grade_component_id} value={edits[editKey(row.registration_id, component.grade_component_id)]?.value ?? ''} max={component.max_mark} error={errors[index]} dirty={edits[editKey(row.registration_id, component.grade_component_id)]?.dirty} disabled={!editable || saving || submitting} onChange={value => updateMark(row.registration_id, component.grade_component_id, value)} />)}{selectedPart === 'theoretical' && <PracticalReference row={row} required={workflow.parts?.practical?.required === true} definitions={workflow.components?.practical ?? []} status={workflow.parts?.practical?.status} />}<td className="px-3 py-3 text-center">{dirty ? <span className="text-amber-700 font-bold text-[11px]">غير محفوظ</span> : <span className="text-green-700 font-bold text-[11px]"><FaCheck className="inline ml-1" />محفوظ</span>}</td></tr> })}
      </tbody></table></div></div>}
...
function OfferingContext({ offering, workflow }) { const year = offering?.academic_year?.year_name || offering?.academicYear?.year_name || offering?.academic_year?.name || offering?.academicYear?.name; const semester = offering?.semester?.semester_name || offering?.semester?.name; return <div className="bg-white border border-primary/12 rounded-[14px] p-4 mb-4" dir="rtl"><div className="font-black text-text-dark">{workflow.course?.course_name || '—'} <span className="font-mono text-[12px] text-text-light">({workflow.course?.course_code || '—'})</span></div><div className="text-[12px] text-text-light mt-1">{[year && `العام: ${year}`, semester && `الفصل: ${semester}`, offering?.section_number && `الشعبة: ${offering.section_number}`].filter(Boolean).join(' · ') || 'بيانات الفصل والسنة غير متاحة'}</div></div> }
function PartStatus({ part, state }) { const required = state?.required === true; const status = required ? STATUSES[state?.status] : null; return <div className={`border rounded-[14px] p-4 ${status?.style || 'bg-gray-50 text-gray-700 border-gray-200'}`} dir="rtl"><div className="flex flex-wrap items-center justify-between gap-2"><div className="font-extrabold text-[14px]">حالة {PARTS[part].short}: {required ? (status?.label || 'غير معروفة') : 'غير مطلوب'}</div>{required && <span className="text-[11px] font-bold">{state?.can_edit ? 'قابل للتحرير' : 'مقفل للقراءة فقط'}</span>}</div>{state?.submitted_at && <div className="text-[11px] mt-2">تاريخ الإرسال: <span dir="ltr">{formatDate(state.submitted_at)}</span></div>}{state?.reviewed_at && <div className="text-[11px] mt-1">تاريخ المراجعة: <span dir="ltr">{formatDate(state.reviewed_at)}</span></div>}</div> }
function ReviewNotes({ part, notes }) { return <div className="mb-4 bg-red-50 border border-red-200 text-red-700 rounded-[12px] p-4 text-[13px]" dir="rtl"><strong>ملاحظات إعادة الجزء {PARTS[part].short} للتصحيح:</strong> {notes}</div> }
function PracticalReference({ row, required, definitions, status }) { if (!required) return <td className="px-3 py-3 text-center font-bold text-text-light">غير مطلوب</td>; const saved = row.marks?.practical ?? []; const marks = definitions.map(component => ({ ...component, mark: saved.find(item => Number(item.grade_component_id) === Number(component.grade_component_id))?.mark ?? null })); const complete = marks.length > 0 && marks.every(item => item.mark !== null && item.mark !== ''); const total = complete ? marks.reduce((sum, item) => sum + Number(item.mark), 0) : null; const max = marks.reduce((sum, item) => sum + Number(item.max_mark), 0); return <td className="px-3 py-3 text-center"><div className="font-bold">{complete ? `${total} / ${max}` : 'غير مكتمل'}</div>{marks.length > 1 && <div className="text-[10px] text-text-light mt-1">{marks.map((item, index) => `${index + 1}: ${item.mark ?? '—'} / ${item.max_mark}`).join('، ')}</div>}<span className={`inline-block mt-1 px-2 py-0.5 rounded-full border text-[10px] font-bold ${STATUSES[status]?.style || 'bg-gray-50 border-gray-200'}`}>{STATUSES[status]?.label || 'غير معروفة'}</span></td> }
