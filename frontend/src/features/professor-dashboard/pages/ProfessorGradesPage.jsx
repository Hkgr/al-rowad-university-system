...
const hasGrade = row => row.theoretical_mark !== null || row.practical_mark !== null
...
export default function ProfessorGradesPage() {
  ...
  useEffect(() => {
    if (!selectedId) { setSheet(null); setWorkflow(null); setEdits({}); setLoadError(''); return }
  ...
  async function refreshPreservingFailures(failedIds) {
    try {
      const [nextSheet, nextWorkflow] = await Promise.all([getGradeSheet(selectedId), getGradeWorkflow(selectedId)])
      const fresh = initialEdits(rowsFromSheet(nextSheet))
      setSheet(nextSheet); setWorkflow(nextWorkflow)
      setEdits(current => Object.fromEntries(Object.keys(fresh).map(id => [id, failedIds.includes(id) ? current[id] : fresh[id]])))
  ...
  async function handleSave() {
    const candidates = rows.filter(row => {
      ...
      return edits[id]?.dirty && !row.is_deprived && row.grade_entry_allowed === true && !validation(edits[id]).theoretical_mark && !validation(edits[id]).practical_mark
    })
    if (!candidates.length) { setNotice({ type: 'error', text: 'لا توجد تعديلات صالحة للحفظ.' }); return }
    ...
    const results = await Promise.all(candidates.map(async row => {
      ...
      try {
        await saveRegistrationGrade(id, { theoretical_mark: Number(edit.theoretical_mark), practical_mark: Number(edit.practical_mark), ...(edit.notes ? { notes: edit.notes } : {}) }, hasGrade(row))
        return { id, ok: true }
      } catch (error) { return { id, ok: false, error } }
    }))
    const failed = results.filter(result => !result.ok)
    const locked = failed.find(result => result.error?.errorCode === 'grades_locked')
    await refreshPreservingFailures(failed.map(result => result.id))
    setSaving(false)
    setNotice({ type: failed.length ? 'error' : 'success', text: `تم حفظ ${results.length - failed.length} سجل، وفشل ${failed.length} سجل.${failed[0] ? ` ${apiMessage(failed[0].error)}` : ''}` })
  ...
  async function handleSubmit() {
    ...
    try {
      await submitOfferingGrades(selectedId)
      setConfirmOpen(false); await loadGrades(selectedId)
      setNotice({ type: 'success', text: 'تم إرسال العلامات إلى هيئة الامتحانات.' })
    } catch (error) {
      if (error.errorCode === 'grade_sheet_incomplete') setIncompleteIds(error.data?.registration_ids ?? [])
      if (error.errorCode === 'grades_locked') await loadGrades(selectedId)
  ...
  return <>
    ...
    {!offeringsLoading && offerings.length > 0 && <div className="grid grid-cols-3 max-[900px]:grid-cols-2 max-[600px]:grid-cols-1 gap-4 mb-6">
      {offerings.map(o => { const active = Number(selectedId) === Number(o.course_offering_id); return <button key={o.course_offering_id} onClick={() => setSelectedId(active ? null : o.course_offering_id)} className={`text-right bg-white border rounded-[16px] px-5 py-4 flex items-center gap-3 shadow-[0_2px_10px_rgba(26,46,16,0.05)] transition-all ${active ? 'border-primary' : 'border-primary/12 hover:-translate-y-[2px]'}`} dir="rtl">
    ...
    {selectedId && !loading && sheet && workflow && <>
      ...
      {rows.length === 0 ? <Empty text="لا يوجد طلاب مسجلون في هذه المادة" /> : <div className="bg-white border border-primary/12 rounded-[16px] overflow-hidden shadow-[0_2px_10px_rgba(26,46,16,0.05)] mb-4"><div className="overflow-x-auto"><table className="w-full min-w-[850px] border-collapse text-[13px]" dir="rtl"><thead><tr className="bg-[#fafaf8]">{['الرقم الجامعي','اسم الطالب','النظري /60','العملي /40','المجموع','الحالة / التقدير','حالة الحفظ'].map(h => <th key={h} className="px-3 py-3 text-center text-[11px] font-bold text-text-light">{h}</th>)}</tr></thead><tbody>
        {rows.map(row => { const id = String(row.student_course_registration_id); const edit = edits[id] || {}; const errors = validation(edit); const editable = globallyEditable && row.grade_entry_allowed === true && !row.is_deprived; const total = edit.theoretical_mark !== '' && edit.practical_mark !== '' ? Number(edit.theoretical_mark) + Number(edit.practical_mark) : null; const incomplete = incompleteIds.some(item => Number(item) === Number(id)); return <tr key={id} className={`border-t border-primary/6 ${incomplete ? 'bg-red-50' : ''}`}>
          ...
          <MarkInput value={edit.theoretical_mark ?? ''} max={60} error={errors.theoretical_mark} disabled={!editable || saving || submitting} onChange={value => updateField(id, 'theoretical_mark', value)} />
          <MarkInput value={edit.practical_mark ?? ''} max={40} error={errors.practical_mark} disabled={!editable || saving || submitting} onChange={value => updateField(id, 'practical_mark', value)} />
...
function MarkInput({ value, max, error, disabled, onChange }) { return <td className="px-3 py-3 align-top"><input type="number" min="0" max={max} step="0.5" value={value} disabled={disabled} onChange={e => onChange(e.target.value)} className={`block mx-auto w-[88px] px-2 py-2 border rounded-[8px] text-center outline-none disabled:bg-gray-100 ${error && value !== '' ? 'border-red-400 bg-red-50' : 'border-primary/20 focus:border-primary'}`} dir="ltr" />{error && value !== '' && <div className="text-[10px] text-red-500 text-center mt-1">{error}</div>}</td> }
