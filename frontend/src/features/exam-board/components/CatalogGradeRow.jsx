...
export default function CatalogGradeRow({ course, change, ...props }) {
  ...
  const { offering, registration, attempts, choices } = selectedContext(course, offeringId, registrationId)
  const context = <div className="space-y-2">
    {choices.length > 1 && <label>اختر سجل المقرر المقصود<select className={field} disabled={props.readOnly && !props.historyMode} value={offering?.course_offering_id ?? ''}
      onChange={e => { const value = e.target.value; change(() => { setOfferingId(value); setRegistrationId('') }) }}>
      <option value="">توجد سجلات متعددة؛ حدد المقصود</option>{choices.map(o => <option key={o.course_offering_id} value={o.course_offering_id}>{o.academic_year} / {o.semester} — {o.program} — السجل {o.course_offering_id} ({o.status})</option>)}
    </select></label>}
    {attempts.length > 1 && <label>التسجيل / المحاولة<select className={field} disabled={props.readOnly && !props.historyMode} value={registration?.registration_id ?? ''}
  ...
  return <RowMode key={`${props.draftEpoch}:${props.term?.academic_year_id}:${props.term?.semester_id}:${offeringId}:${registrationId}`}
    ...
    ambiguous={(!offering && choices.length > 1) || (!registration && attempts.length > 1)} />
}
