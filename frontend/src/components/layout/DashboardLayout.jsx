 ...
 * nav prop shape:
 * [
 *   { label: 'القائمة الرئيسية', items: [{ to, Icon, ar, en }] },
...
export default function DashboardLayout({ nav = [], appTitle = 'جامعة الرواد' }) {
  ...
  const renderSection = (section, idx) => (
    <div key={idx} className="flex flex-col gap-0.5 pb-0.5">
      {!collapsed && (
  ...
  return (
    <div className="flex min-h-screen bg-[#f0f5ec]" dir="rtl">
      ...
      >
        <div className="absolute inset-0 pointer-events-none z-0" style={{ backgroundImage: 'radial-gradient(circle, rgba(86,153,51,0.1) 1px, transparent 1px)', backgroundSize: '22px 22px' }} />

        <div className="relative z-[1] flex flex-col h-full">

          {/* Top accent bar */}
          <div className="h-1 flex-shrink-0 animate-[barFlow_3.5s_linear_infinite]" style={{ background: 'linear-gradient(90deg, #417327, #7ab356, #a8d68a, #7ab356, #417327)', backgroundSize: '200% 100%' }} />

          {/* Logo */}
          <div className="flex items-center gap-3 px-4 py-[18px] min-h-[82px] border-b border-white/6 overflow-hidden" dir="rtl">
            <div className="flex-shrink-0 w-[46px] h-[46px] rounded-full p-[2px] animate-[ringRotate_6s_linear_infinite] shadow-[0_0_18px_rgba(86,153,51,0.45)]" style={{ background: 'conic-gradient(from 0deg, #569933, #7ab356, #a8d68a, #569933)' }}>
              <img src="/logo.png" alt="Logo" className="w-full h-full rounded-full object-contain bg-white p-1 block" />
            </div>
            {!collapsed && (
              <div className="flex flex-col min-w-0 overflow-hidden">
                <span className="text-[12.5px] font-black text-white whitespace-nowrap overflow-hidden text-ellipsis tracking-[0.3px]">{appTitle}</span>
                <span className="text-[9.5px] font-normal text-white/40 whitespace-nowrap overflow-hidden text-ellipsis mt-[3px] tracking-[0.4px]">Alrowad University</span>
              </div>
            )}
          </div>
          ...
          {/* Collapse toggle */}
          <button
            className="flex items-center justify-center my-[7px] mx-auto w-8 h-8 border border-white/9 rounded-[9px] bg-white/4 text-white/40 cursor-pointer transition-all duration-[220ms] flex-shrink-0 hover:bg-primary/20 hover:border-primary/38 hover:text-white"
            onClick={() => setCollapsed(v => !v)}
            title={collapsed ? 'توسيع القائمة' : 'طي القائمة'}
          >
            <FaChevronRight className={`text-[13px] transition-transform duration-300 ${!collapsed ? 'rotate-180' : ''}`} />
          </button>
